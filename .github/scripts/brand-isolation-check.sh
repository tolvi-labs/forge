...
# Terms this repo is allowed to say, removed from the pattern rather than whole
# files being skipped, so an allowlisted file is still checked for everything
# else. Firebase and Firestore are on the shared blocklist because naming them
# elsewhere would hint at a sibling deployment's stack. Here they are a shipped
# feature: Forge has a fullstack-firebase profile, MCP server configs that wire
# it up, and tests asserting on both. Renaming a public technology in a public
# feature would damage the product to protect nothing a reader could not guess.
ALLOWLIST_TERMS=(firebase firestore)

PATTERN="$BRAND_BLOCKLIST"
for term in "${ALLOWLIST_TERMS[@]}"; do
  # Strip the term and any word-boundary form of it from the alternation.
  PATTERN="$(printf '%s' "$PATTERN" | sed -E "s/\\|?\\\\b${term}\\\\b//g; s/\\|?${term}//g")"
done
PATTERN="$(printf '%s' "$PATTERN" | sed -E 's/^\|+//; s/\|+$//; s/\|\|+/|/g')"
